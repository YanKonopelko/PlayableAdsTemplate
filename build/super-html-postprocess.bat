...
REM 3) For all .html files in build/super-html/ironsourcemraid/ replace
REM    window.super_html_channel = "ironsourcemraid" with "unity".
REM
REM Usage:
...
set "IRONSOURCE_MRAID_DIR=%SCRIPT_DIR%super-html\ironsourcemraid"
...
set /a IRONSOURCE_MRAID_TOTAL=0
set /a IRONSOURCE_MRAID_CHANGED=0
set /a IRONSOURCE_MRAID_SKIPPED=0
set /a IRONSOURCE_MRAID_ERRORS=0
...
REM Process Ironsource MRAID files
if exist "%IRONSOURCE_MRAID_DIR%" (
    echo Processing Ironsource MRAID files...
    for %%f in ("%IRONSOURCE_MRAID_DIR%\*.html") do (
        set /a IRONSOURCE_MRAID_TOTAL+=1
        call :process_ironsource_mraid_file "%%f"
        set RESULT=!errorlevel!
        if !RESULT! equ 3 (
            set /a IRONSOURCE_MRAID_ERRORS+=1
        ) else if !RESULT! equ 2 (
            set /a IRONSOURCE_MRAID_SKIPPED+=1
        ) else if !RESULT! equ 1 (
            set /a IRONSOURCE_MRAID_CHANGED+=1
        )
    )
) else (
    echo [WARNING] Ironsource MRAID directory not found: %IRONSOURCE_MRAID_DIR%
)
...
echo   Ironsource MRAID: Total=!IRONSOURCE_MRAID_TOTAL! Changed=!IRONSOURCE_MRAID_CHANGED! Skipped=!IRONSOURCE_MRAID_SKIPPED! Errors=!IRONSOURCE_MRAID_ERRORS!
echo.

set EXIT_CODE=0
if !UNITY_ERRORS! GTR 0 set EXIT_CODE=1
if !IRONSOURCE_ERRORS! GTR 0 set EXIT_CODE=1
if !IRONSOURCE_MRAID_ERRORS! GTR 0 set EXIT_CODE=1
...
REM Function to process Ironsource MRAID HTML file
REM Returns: 1=changed, 2=skipped, 3=error
:process_ironsource_mraid_file
set "FILE_PATH=%~1"
set "TEMP_PS=%TEMP%\postprocess_ironsourcemraid_%RANDOM%.ps1"
(
echo $ErrorActionPreference='Stop'
echo try ^{
echo     $filePath = "%FILE_PATH%"
echo     $search = "window.super_html_channel = " + [char]34 + "ironsourcemraid" + [char]34
